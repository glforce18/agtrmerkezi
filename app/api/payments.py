...
def create_notification(
    db: Session, user_id: int, type: str, title: str, message: str, link: str = None
):
    """Bildirim olustur"""
    try:
...
async def select_payment_method(
    ...
    if data.coupon_code and not payment.coupon_code:
        ...
        if coupon:
            ...
            if coupon.valid_from and coupon.valid_from > now:
                ...
                raise HTTPException(
                    status_code=400,
                    detail=f"Bu kupon minimum {coupon.min_amount} TL odeme icin gecerli",
                )
            else:
    ...
    if method == PaymentMethod.BALANCE:
        if current_user.balance < payment.amount:
            raise HTTPException(
                status_code=400,
                detail=f"Yetersiz bakiye. Mevcut: {current_user.balance} TL, Gerekli: {payment.amount} TL",
            )
        ...
        try:
            ...
            if locked_user.balance < payment.amount:
                raise HTTPException(
                    status_code=400,
                    detail=f"Yetersiz bakiye. Mevcut: {locked_user.balance} TL, Gerekli: {payment.amount} TL",
                )
            ...
            if payment.server_id:
                server = (
                    db.query(GameServer)
                    .filter(GameServer.id == payment.server_id)
                    .with_for_update()
                    .first()
                )
                if server:
            ...
            return {
                "success": True,
                "message": "Odeme bakiyeden alindi",
...
async def notify_bank_transfer(
    ...
    existing = (
        ...
        .filter(BankTransfer.payment_id == payment.id)
        .with_for_update()
        .first()
    )
    if existing:
...
async def add_balance(
    ...
    if data.amount < settings.BALANCE_MIN_AMOUNT:
        raise HTTPException(
            status_code=400, detail=f"Minimum yukleme tutari {settings.BALANCE_MIN_AMOUNT} TL"
        )
    if data.amount > settings.BALANCE_MAX_AMOUNT:
        raise HTTPException(
            status_code=400, detail=f"Maksimum yukleme tutari {settings.BALANCE_MAX_AMOUNT} TL"
        )
...
async def apply_coupon(
    ...
    if coupon.min_amount and payment.amount < coupon.min_amount:
        raise HTTPException(
            status_code=400, detail=f"Bu kupon minimum {coupon.min_amount} TL odeme icin gecerli"
        )
...
async def remove_coupon(
    ...
    log_audit(
        ...
        "payment",
        payment_id,
        old_values={"coupon_code": old_coupon},
        ip_address=client_ip,
    )
...
async def get_invoices(
    ...
    total = query.count()
    invoices = (
        query.order_by(desc(Invoice.created_at)).offset((page - 1) * per_page).limit(per_page).all()
    )
...
async def toggle_auto_renew(
    ...
    server = (
        db.query(GameServer)
        .filter(GameServer.id == data.server_id, GameServer.owner_id == current_user.id)
        .first()
    )
...
async def cancel_payment(
    ...
    client_ip = request.client.host if request.client else None
    log_audit(db, current_user.id, "payment_cancel", "payment", payment_id, ip_address=client_ip)
