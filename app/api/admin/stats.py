...
async def get_user_stats(
    ...
    # Steam linked users
    steam_linked = db.query(func.count(User.id)).filter(
        User.steam_id != None,
        User.steam_id != ""
    ).scalar() or 0

    # Email verified users
    email_verified = db.query(func.count(User.id)).filter(
    ...
    two_fa_enabled = db.query(func.count(User.id)).filter(
    ...
    week_registrations = db.query(func.count(User.id)).filter(
        User.created_at >= week_start
    ).scalar() or 0

    # Users registered this month
    month_start = datetime.utcnow().replace(day=1, hour=0, minute=0, second=0, microsecond=0)
    month_registrations = db.query(func.count(User.id)).filter(
    ...
    return {
        ...
        "email_verified_percentage": round(email_verified / total_users * 100, 1) if total_users > 0 else 0,
...
async def get_steam_breakdown(
    ...
    steam_with_email = db.query(func.count(User.id)).filter(
        User.steam_id != None,
        User.steam_id != "",
        User.email_verified == True
    ).scalar() or 0
    ...
    steam_paying_users = db.query(func.count(func.distinct(Payment.user_id))).join(
    ...
    non_steam_paying_users = db.query(func.count(func.distinct(Payment.user_id))).join(
        User, User.id == Payment.user_id
    ).filter(
    ...
    steam_server_owners = db.query(func.count(func.distinct(GameServer.owner_id))).join(
        User, User.id == GameServer.owner_id
    ).filter(
        User.steam_id != None,
        User.steam_id != "",
    ...
    steam_role_distribution = db.query(
        User.role,
        func.count(User.id)
    ).filter(
        User.steam_id != None,
    ...
    return {
        ...
        "steam_engagement": {
            ...
            "paying_percentage": round(steam_paying_users / steam_linked * 100, 1) if steam_linked > 0 else 0
        },
        "non_steam_engagement": {
            "paying_users": non_steam_paying_users,
            "paying_percentage": round(non_steam_paying_users / (total_users - steam_linked) * 100, 1) if (total_users - steam_linked) > 0 else 0
        },
...
async def get_registration_stats(
    ...
    for i in range(days):
        ...
        day_count = db.query(func.count(User.id)).filter(
            func.date(User.created_at) == day
        ).scalar() or 0

        # Steam registrations for the day
        steam_count = db.query(func.count(User.id)).filter(
            func.date(User.created_at) == day,
            User.steam_id != None,
            User.steam_id != ""
        ).scalar() or 0
        ...
        non_steam_daily.append({
            "date": str(day),
            "count": day_count - steam_count
        })
    ...
    # Monthly summary
    monthly_total = sum(r["count"] for r in daily_registrations[-30:] if len(daily_registrations) >= 30) or sum(r["count"] for r in daily_registrations)
    monthly_steam = sum(r["count"] for r in steam_daily[-30:] if len(steam_daily) >= 30) or sum(r["count"] for r in steam_daily)
    ...
    return {
        ...
        "summary": {
            "weekly": {
                ...
                "steam_percentage": round(weekly_steam / weekly_total * 100, 1) if weekly_total > 0 else 0
            },
            "monthly": {
                "total": monthly_total,
                "steam": monthly_steam,
                "non_steam": monthly_total - monthly_steam,
                "steam_percentage": round(monthly_steam / monthly_total * 100, 1) if monthly_total > 0 else 0
            }
        }
    }
...
async def get_activity_stats(
    ...
    active_in_period = db.query(func.count(User.id)).filter(
        User.last_login >= start_date
    ).scalar() or 0

    # Steam users active in period
    steam_active = db.query(func.count(User.id)).filter(
        User.last_login >= start_date,
        User.steam_id != None,
        User.steam_id != ""
    ).scalar() or 0
    ...
    # Logins by type
    login_types = db.query(
        LoginHistory.login_type,
        func.count(LoginHistory.id)
    ).filter(
        LoginHistory.created_at >= start_date,
        LoginHistory.is_successful == True
    ).group_by(LoginHistory.login_type).all()

    login_type_breakdown = {login_type: count for login_type, count in login_types}

    # Forum activity
    new_topics = db.query(func.count(ForumTopic.id)).filter(
        ForumTopic.created_at >= start_date
    ).scalar() or 0

    new_posts = db.query(func.count(ForumPost.id)).filter(
        ForumPost.created_at >= start_date
    ).scalar() or 0

    # Forum activity by Steam users
    steam_topics = db.query(func.count(ForumTopic.id)).join(
        User, User.id == ForumTopic.author_id
    ).filter(
        ForumTopic.created_at >= start_date,
        User.steam_id != None,
        User.steam_id != ""
    ...
    steam_posts = db.query(func.count(ForumPost.id)).join(
        User, User.id == ForumPost.author_id
    ).filter(
        ForumPost.created_at >= start_date,
        User.steam_id != None,
        User.steam_id != ""
    ...
    steam_revenue = db.query(func.sum(Payment.amount)).join(
        User, User.id == Payment.user_id
    ).filter(
        Payment.completed_at >= start_date,
        Payment.status == PaymentStatus.COMPLETED,
    ...
    # Server activity
    new_servers = db.query(func.count(GameServer.id)).filter(
        GameServer.created_at >= start_date
    ).scalar() or 0
    ...
    return {
        ...
        "user_activity": {
            ...
            "steam_active_percentage": round(steam_active / active_in_period * 100, 1) if active_in_period > 0 else 0,
        ...
        "forum_activity": {
            ...
            "steam_forum_percentage": round((steam_topics + steam_posts) / (new_topics + new_posts) * 100, 1) if (new_topics + new_posts) > 0 else 0
        },
        "financial_activity": {
            ...
            "steam_revenue_percentage": round(float(steam_revenue or 0) / float(total_revenue) * 100, 1) if total_revenue > 0 else 0
        },
        "server_activity": {
...
async def get_top_spenders(
    ...
    top_spenders = db.query(
        ...
        User.id, User.username, User.steam_id, User.email
    ).order_by(
