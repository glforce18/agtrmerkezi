...
async def admin_dashboard(db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    new_users_today = (
        db.query(User).filter(func.date(User.created_at) == datetime.utcnow().date()).count()
    )
...
async def admin_dashboard_charts(
    ...
    for i in range(days):
        ...
        # Kullanıcı kayıtları
        user_count = db.query(User).filter(func.date(User.created_at) == day).count()
        ...
        day_revenue = (
            db.query(func.sum(Payment.amount))
            .filter(
                Payment.status == PaymentStatus.COMPLETED, func.date(Payment.completed_at) == day
            )
            .scalar()
            or 0
        )
    ...
    for game in GameType:
        count = (
            db.query(GameServer)
            .filter(GameServer.game_type == game, GameServer.status != ServerStatus.DELETED)
            .count()
        )
        if count > 0:
    ...
    payment_method_dist = (
        db.query(Payment.method, func.count(Payment.id), func.sum(Payment.amount))
        .filter(Payment.status == PaymentStatus.COMPLETED)
    ...
    methods = [
        {"method": m.value if m else "other", "count": c, "total": float(t or 0)}
...
async def list_users(
    ...
    total = query.count()
    users = (
        query.order_by(desc(User.created_at)).offset((page - 1) * per_page).limit(per_page).all()
    )
...
async def get_user(
    ...
    return {
        ...
        "servers": [{"id": s.id, "name": s.name, "status": s.status.value} for s in servers],
        "recent_payments": [
            {
                "id": p.id,
                "amount": p.amount,
...
async def update_user(
    ...
    if data.role:
        try:
            ...
            if (
                new_role in [UserRole.ADMIN, UserRole.SUPERADMIN]
...
async def list_servers(
    ...
    total = query.count()
    servers = (
        query.order_by(desc(GameServer.created_at))
        ...
        result.append(
            {
                ...
                "last_query": (
                    s.updated_at.isoformat() if hasattr(s, "updated_at") and s.updated_at else None
                ),
                "resources": (
                    {
                        "cpu": getattr(s, "cpu_usage", 0) or 0,
                        "ram": getattr(s, "ram_usage", 0) or 0,
                        "disk": 0,
                    }
                    if hasattr(s, "cpu_usage") or hasattr(s, "ram_usage")
                    else None
                ),
            }
        )
...
async def list_payments(
    ...
    total = query.count()
    payments = (
        query.order_by(desc(Payment.created_at)).offset((page - 1) * per_page).limit(per_page).all()
    )
...
async def delete_package(
    ...
    if active_servers > 0:
        raise HTTPException(
            status_code=400, detail=f"Bu pakete bagli {active_servers} aktif sunucu var"
        )
...
async def update_settings(
    ...
    if admin.role != UserRole.SUPERADMIN:
        raise HTTPException(
            status_code=403, detail="Site ayarlarini sadece superadmin degistirebilir"
        )
    ...
    log = SystemLog(
        level="info", category="admin", message="Site ayarlari guncellendi", user_id=admin.id
    )
    db.add(log)
    db.commit()
...
async def get_plugin(
    plugin_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
):
    """Plugin detayi"""
...
async def delete_plugin(
    ...
    if usage_count > 0:
        raise HTTPException(
            status_code=400,
            detail=f"Bu plugin {usage_count} sunucuda kullaniliyor. Once sunuculardan kaldirin.",
        )
...
async def assign_plugin_to_server(
    ...
    server_plugin = ServerPlugin(
        server_id=server_id, plugin_id=plugin_id, is_enabled=True, installed_by=admin.id
    )
...
async def list_scheduled_tasks(
    ...
    total = query.count()
    tasks = (
        query.order_by(desc(ScheduledTask.created_at))
        .offset((page - 1) * per_page)
        .limit(per_page)
        .all()
    )
    ...
    for task in tasks:
        server = (
            db.query(GameServer).filter(GameServer.id == task.server_id).first()
            if task.server_id
            else None
        )
        result.append(
...
async def list_backups(
    ...
    total = query.count()
    backups = (
        query.order_by(desc(BackupLog.created_at))
        .offset((page - 1) * per_page)
        .limit(per_page)
        .all()
    )
    ...
    for b in backups:
        server = (
            db.query(GameServer).filter(GameServer.id == b.server_id).first()
            if b.server_id
            else None
        )
        result.append(
...
async def create_backup(
    ...
    if server_id:
        ...
        try:
            ...
            dump_result = subprocess.run(mysqldump_cmd, capture_output=True, env=env, timeout=300)
    ...
    return {
        ...
        "backup": {"filename": backup_name, "size_mb": round(file_size / 1024 / 1024, 2)},
    }
...
async def delete_backup(
    ...
    log_admin_audit(
        ...
        "backup",
        backup_id,
        old_values={"file_path": backup.file_path},
        ip_address=client_ip,
    )
...
async def require_user_2fa(
    ...
    log_admin_audit(
        ...
        "user",
        user_id,
        new_values={"require_2fa": data.require_2fa},
        ip_address=client_ip,
    )
...
async def get_system_stats(db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
    ...
    try:
        ...
        return {
            ...
            "activity": {"audit_logs_24h": audit_count_24h, "active_sessions": active_sessions},
...
async def save_theme(
    theme: ThemeSettings, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
):
...
async def get_user_details(
    ...
    return {
        ...
        "recent_activity": [
            {"action": a.action, "created_at": a.created_at.isoformat() if a.created_at else None}
            for a in audits
        ],
    }
...
async def admin_reset_user_password(
    ...
    return {
        "success": True,
        "message": f"{user.email} adresine sifre sifirlama e-postasi gonderildi",
    }
        ...
        "tickets": [
            {
                ...
                "unread_count": db.query(TicketMessage)
                .filter(
...
async def get_ticket_stats(db: Session = Depends(get_db), admin: User = Depends(get_current_admin)):
...
async def get_ticket(
    ...
    db.query(TicketMessage).filter(
        TicketMessage.ticket_id == ticket_id, TicketMessage.is_admin == False
    ).update({"is_read": True})
    db.commit()
        ...
        "messages": [
            {
                ...
                or (ticket.user.username if ticket.user and not m.is_admin else "Admin"),
                "is_admin": m.is_admin,
...
async def reply_to_ticket(
    ...
    message = TicketMessage(
        ticket_id=ticket_id, content=data.message, is_admin=True, author_name=admin.username
    )
    db.add(message)
...
async def delete_banner(
    banner_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
):
    """Banner sil"""
    banner = db.query(Banner).filter(Banner.id == banner_id).first()
    if not banner:
...
@router.get("/media")
async def list_media(admin: User = Depends(get_current_admin)):
...
async def upload_media(
    ...
    if ext not in allowed_ext:
        raise HTTPException(
            status_code=400,
            detail="Gecersiz dosya formati. Izin verilen: png, jpg, jpeg, gif, webp, svg",
        )
...
async def delete_media(
    media_id: int, file_path: str = Query(...), admin: User = Depends(get_current_admin)
):
    """Gorsel sil"""
    import os
...
async def get_dashboard_stats(
    ...
    # Toplam sunucu
    total_servers = db.query(func.count(GameServer.id)).scalar() or 0

    # Aktif sunucular (RUNNING status)
    active_servers = (
    ...
    revenue_today = (
        db.query(func.sum(Payment.amount))
        .filter(Payment.status == PaymentStatus.COMPLETED, func.date(Payment.created_at) == today)
        .scalar()
        or 0
    )
    ...
    # Bekleyen ödemeler
    pending_payments = (
        db.query(func.count(Payment.id)).filter(Payment.status == PaymentStatus.PENDING).scalar()
        or 0
    )
...
async def get_recent_payments(
    ...
    Son ödemeler
    """
    payments = db.query(Payment).join(User).order_by(Payment.created_at.desc()).limit(limit).all()

    return [
        {
            "id": p.id,
            "user_id": p.user_id,
            "user_username": p.user.username if p.user else "Unknown",
            "amount": float(p.amount),
...
async def restart_server_admin(
    ...
    try:
        ...
        rcon = RCONService()
        await rcon.execute_command(server, "_restart")

        return {"success": True, "message": "Sunucu yeniden başlatılıyor"}
    except Exception as e:
        logger.error(f"Server restart error: {e}")
        raise HTTPException(status_code=500, detail=f"Restart hatası: {str(e)}")


@router.post("/servers/{server_id}/stop")
async def stop_server_admin(
    server_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
):
    """Admin: Sunucu durdur"""
    server = db.query(GameServer).filter(GameServer.id == server_id).first()
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Stop hatası: {str(e)}")


@router.delete("/servers/{server_id}")
async def delete_server_admin(
    server_id: int, db: Session = Depends(get_db), admin: User = Depends(get_current_admin)
):
    """Admin: Sunucu sil"""
    server = db.query(GameServer).filter(GameServer.id == server_id).first()
    ...
    try:
        # Stop and delete physical server
        delete_physical_server(server.id)
...
class BulkServerRequest(BaseModel):
    server_ids: List[int]
...
@router.post("/servers/bulk/restart")
async def bulk_restart_servers(
    request: BulkServerRequest,
    db: Session = Depends(get_db),
    admin: User = Depends(get_current_admin),
):
    """Toplu sunucu yeniden başlatma"""
    ...
    for server_id in request.server_ids:
        try:
            ...
            audit = AuditLog(
                user_id=admin.id,
                action="bulk_server_restart",
                resource_type="server",
                resource_id=server.id,
                details=f"Bulk restart: {server.name}",
            )
            db.add(audit)

        except Exception as e:
            logger.error(f"Bulk restart error for server {server_id}: {e}")
            results["failed"] += 1
...
@router.post("/servers/bulk/stop")
async def bulk_stop_servers(
    request: BulkServerRequest,
    db: Session = Depends(get_db),
    admin: User = Depends(get_current_admin),
):
    """Toplu sunucu durdurma"""
    results = {"completed": 0, "failed": 0, "errors": []}
    ...
    for server_id in request.server_ids:
        try:
            ...
            audit = AuditLog(
                user_id=admin.id,
                action="bulk_server_stop",
                resource_type="server",
                resource_id=server.id,
                details=f"Bulk stop: {server.name}",
            )
            db.add(audit)

        except Exception as e:
            logger.error(f"Bulk stop error for server {server_id}: {e}")
            results["failed"] += 1
            results["errors"].append(f"Server {server_id}: {str(e)}")
...
@router.post("/servers/bulk/command")
async def bulk_send_command(
    request: BulkCommandRequest,
    db: Session = Depends(get_db),
    admin: User = Depends(get_current_admin),
):
    """Toplu RCON komut gönderme"""
    results = {"completed": 0, "failed": 0, "errors": [], "responses": []}
    ...
    for server_id in request.server_ids:
        try:
            ...
            response = await rcon.execute_command(server, request.command)
            results["completed"] += 1
            results["responses"].append(
                {"server_id": server_id, "server_name": server.name, "response": response}
            )

            # Audit log
            audit = AuditLog(
                user_id=admin.id,
                action="bulk_rcon_command",
                resource_type="server",
                resource_id=server.id,
                details=f"Bulk command: {request.command} on {server.name}",
            )
            db.add(audit)

        except Exception as e:
            logger.error(f"Bulk command error for server {server_id}: {e}")
            results["failed"] += 1
            results["errors"].append(f"Server {server_id}: {str(e)}")
