...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def cleanup_old_requests(self, ip: str):
        ...
        self.minute_requests[ip] = [ts for ts in self.minute_requests[ip] if now - ts < 60]

        # 1 saniyeden eski kayitlari sil
        self.second_requests[ip] = [ts for ts in self.second_requests[ip] if now - ts < 1]
    ...
    async def aggressive_cleanup(self):
        ...
        self.blocked_ips = {
            ip: unblock_time for ip, unblock_time in self.blocked_ips.items() if unblock_time > now
        }
        ...
        if total_ips > self.CLEANUP_THRESHOLD:
            ...
            logger.info(
                "Aggressive cleanup: removed %d inactive IPs, remaining: %d",
                len(ips_to_remove),
        ...
        if len(self.minute_requests) > self.MAX_TRACKED_IPS:
            ...
            logger.warning(
                f"Emergency cleanup: removed {remove_count} oldest IPs to prevent memory overflow"
            )
    ...
    async def dispatch(self, request: Request, call_next):
        ...
        # Live chat ve logs polling için limit yok (her 5 saniyede polling)
        if "/live-chat" in request.url.path or "/logs" in request.url.path:
            return await call_next(request)
        ...
        if is_upload_path:
            ...
            async with self._lock:
                ...
                self.minute_requests[upload_key] = [
                    t for t in self.minute_requests[upload_key] if now - t < 60
                ]
        ...
        async with self._lock:
            ...
            if limited:
                ...
                return JSONResponse(
                    status_code=429,
                    content={"detail": "Too many requests", "reason": reason, "retry_after": 60},
                    headers={"Retry-After": "60"},
                )
