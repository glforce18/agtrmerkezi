        ...
        if is_admin_path:
            ...
            try:
                ...
                if user.role.value not in ["admin", "superadmin"]:
                    logger.warning(f"Admin erişim denemesi - Yetkisiz: {user.username} -> {path}")
                    return RedirectResponse(url="/panel", status_code=302)
                
                # Superadmin-only path kontrolü
                is_superadmin_path = any(path == p or path.startswith(p + "/") for p in self.SUPERADMIN_ONLY_PATHS)
                if is_superadmin_path and user.role.value != "superadmin":
                    logger.warning(f"Superadmin erişim denemesi - Yetkisiz: {user.username} -> {path}")
...
def require_admin(func):
    """Admin gerektiren route'lar için decorator"""
...
def require_superadmin(func):
    """Superadmin gerektiren route'lar için decorator"""
    async def wrapper(request: Request, *args, **kwargs):
        token = request.cookies.get("access_token")
        if not token:
            return RedirectResponse(url="/login", status_code=302)
